...
for email, us in email_dups.items():
    if len(us) > 1:
        ...
        for u in us:
            print(f"   _id: {u.get('_id')}, username: {u.get('username')}, name: {u.get('name','')} ")
        print(f"  SUGERENCIA: Mantén solo uno y elimina los demás con: db.users.deleteOne({{'_id': ObjectId('...')}})")
...
for username, us in username_dups.items():
    if len(us) > 1:
        ...
        for u in us:
            print(f"   _id: {u.get('_id')}, email: {u.get('email')}, name: {u.get('name','')} ")
        print(f"  SUGERENCIA: Mantén solo uno y elimina los demás con: db.users.deleteOne({{'_id': ObjectId('...')}})")
...
def listar_huerfanos(collection, label):
    ...
    if huérfanos:
        ...
        for h in huérfanos:
            print(f"   - {h}")
        print(f"  SUGERENCIA: Elimina con: db.{collection}.deleteOne({{'_id': ObjectId('...')}}) o reasigna el owner a un usuario válido.")
    else:
