...
def index():
    ...
    return render_template('acceso_fix.html', 
...
@app.route('/crear_cookie_admin')
def crear_cookie_admin():
    """Crea una cookie de sesión para acceder como administrador"""
    try:
        ...
        if not usuario:
            return jsonify({
                "success": False,
                "message": "No se encontró ningún usuario administrador en la base de datos."
            })
        
        # Datos para la cookie de sesión
        session_data = {
            "_permanent": True,
        ...
        # Crear respuesta con instrucciones
        response = jsonify({
            "success": True,
            "message": "Cookie de sesión generada correctamente",
            "cookie_name": app.config['SESSION_COOKIE_NAME'],
            "cookie_value": cookie_value,
            "instructions": "Copia esta cookie y pégala en tu navegador para acceder como administrador."
        })
        ...
        logger.error(f"Error al crear cookie de sesión: {str(e)}")
        return jsonify({
            "success": False,
            "message": f"Error al crear cookie de sesión: {str(e)}"
        })
...
def login_admin(username):
    ...
    try:
        ...
        return redirect('http://edefrutos2025.xyz/admin/')
...
# Crear plantilla acceso_fix.html
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'templates', 'acceso_fix.html'), 'w') as f:
    f.write("""
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="container">
        ...
        <div class="actions">
            ...
            <a href="{{ url_for('crear_cookie_admin') }}" class="btn btn-info">Generar cookie de administrador</a>
        ...
        <div class="instructions">
            ...
            <ol>
                <li>Haz clic en "Acceder como edefrutos (admin)" o "Acceder como administrator (admin)" para iniciar sesión automáticamente.</li>
                <li>Si eso no funciona, haz clic en "Generar cookie de administrador" y sigue las instrucciones.</li>
                <li>Una vez que tengas la sesión activa, visita <a href="http://edefrutos2025.xyz/admin/" target="_blank">http://edefrutos2025.xyz/admin/</a></li>
            </ol>
            
            <h3>Solución alternativa:</h3>
            <p>Si ninguna de las opciones anteriores funciona, puedes intentar lo siguiente:</p>
            <ol>
                <li>Inicia sesión normalmente en <a href="http://edefrutos2025.xyz/login" target="_blank">http://edefrutos2025.xyz/login</a> con las credenciales de administrador.</li>
                <li>Luego, visita directamente <a href="http://edefrutos2025.xyz/admin/" target="_blank">http://edefrutos2025.xyz/admin/</a></li>
            </ol>
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'templates', 'admin_info.html'), 'w') as f:
    f.write("""
<!DOCTYPE html>
<html>
<head>
