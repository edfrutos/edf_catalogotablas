...
if ! command -v python3.8 &> /dev/null; then
    ...
    echo "💡 Instala Python 3.8 con: brew install python@3.8"
    exit 1
fi

# Verificar PyInstaller
if ! python3.8 -c "import PyInstaller" &> /dev/null; then
    echo "📦 Instalando PyInstaller..."
    python3.8 -m pip install pyinstaller==6.15.0 pyinstaller-hooks-contrib==2025.8
fi

# Verificar PyWebView
if ! python3.8 -c "import webview" &> /dev/null; then
    echo "📦 Instalando PyWebView..."
    python3.8 -m pip install pywebview
fi
...
python3.8 -m PyInstaller EDF_CatalogoDeTablas.spec --clean
