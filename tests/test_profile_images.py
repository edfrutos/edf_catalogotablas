        ...
        for user in users:
            if "foto_perfil" in user and user["foto_perfil"]:
                ...
                image_path = os.path.join(
                    app.static_folder, "uploads", user["foto_perfil"]
                )

                # Verificar si el archivo existe
                if os.path.exists(image_path):
                    logger.info(
                        f"✅ Usuario {user.get('email')}: Imagen encontrada en {image_path}"
                    )
                else:
                    logger.error(
                        f"❌ Usuario {user.get('email')}: Imagen no encontrada en {image_path}"
                    )

                # Verificar la ruta en las plantillas
                with app.test_request_context():
                    template_path = url_for(
                        "static", filename=f'uploads/{user["foto_perfil"]}'
                    )
                    logger.info(f"   URL en plantilla: {template_path}")
            else:
                logger.info(
                    f"ℹ️ Usuario {user.get('email')}: No tiene foto de perfil configurada"
                )
