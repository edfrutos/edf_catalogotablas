        ...
        with app.app_context():
            ...
            if admins:
                ...
                for user in admins:
                    ...
                    temp_password = f"{username}123" if username != "N/A" else f"{email.split('@')[0]}123"
                    print(f"   📧 {email}")
            ...
            if regular_users:
                ...
                for i, user in enumerate(regular_users[:5]):  # Solo mostrar 5 como ejemplo
                    username = user.get("username", "N/A")
                    email = user.get("email", "N/A")
                    temp_password = f"{username}123" if username != "N/A" else f"{email.split('@')[0]}123"
                    print(f"   {i+1}. 📧 {email}")
...
def main():
    ...
    if success:
        print(f"\n🌐 URLs DEL SISTEMA")
        print("-" * 50)
        print(f"📱 Página principal: https://edefrutos2025.xyz")
        print(f"🔐 Login normal: https://edefrutos2025.xyz/login")
        print(f"⚠️  Notificación de contraseñas: https://edefrutos2025.xyz/password/password-reset-notification")
        print(f"🔑 Login temporal: https://edefrutos2025.xyz/password/temporary-login")
        print(f"🔄 Cambio forzado: https://edefrutos2025.xyz/usuarios/force_password_change")
