...
try:
    # Primero intentamos cargar desde el archivo de configuración local
    from dev_template.tests.legacy.config_debug import (
        BASE_URL,
        ADMIN_EMAIL,
        ADMIN_PASSWORD,
        MONGO_URI,
    ...
    logger.info("✅ Configuración cargada desde config_debug.py")
except ImportError:
    # Si no existe, intentamos cargar desde variables de entorno o usamos valores por defecto
    logger.warning(
        "⚠️ No se pudo cargar config_debug.py, usando variables de entorno o valores por defecto"
    )
...
def probar_credenciales_alternativas(session):
    """Prueba diferentes combinaciones de credenciales de administrador comunes."""
    credenciales_comunes = [
        ...
        {"email": "superadmin", "password": "superadmin"},
    ]

    logger.info("Probando credenciales alternativas comunes...")

    for creds in credenciales_comunes:
        logger.info(f"Probando: {creds['email']} / {creds['password'][:2]}***")
        success, _ = iniciar_sesion(session, creds["email"], creds["password"])
        if success:
            logger.info(
                f"✅ Inicio de sesión exitoso con credenciales alternativas: {creds['email']}"
            )
            return True, creds

    logger.warning("❌ Todas las credenciales alternativas fallaron")
    return False, None
...
def iniciar_sesion(session, email, password):
    ...
    try:
        ...
        try:
            server_check = session.get(BASE_URL, timeout=5)
            logger.info(
                f"✅ Servidor accesible en {BASE_URL} (Status: {server_check.status_code})"
            )
        except requests.RequestException as e:
            logger.error(f"❌ No se puede acceder al servidor en {BASE_URL}: {str(e)}")
            logger.info(
                "⚠️ Asegúrate de que el servidor Flask esté ejecutándose en el puerto correcto"
            )
            return False, None

        # Obtener la página de login para extraer el token CSRF
        logger.info(f"Accediendo a la página de login: {BASE_URL}/login")
        ...
        logger.info("✅ HTML de la página de login guardado en login_page_debug.html")
        ...
        if not form:
            logger.warning("❌ No se encontró ningún formulario en la página de login")
            return False, None

        logger.info(
            f"Formulario encontrado con action: {form.get('action', 'No especificado')}"
        )
        inputs = form.find_all("input")
        input_names = [i.get("name") for i in inputs if i.get("name")]
        logger.info(f"Campos del formulario: {input_names}")

        # Verificar si el formulario tiene un campo CSRF
        csrf_token = obtener_csrf_token(response.text)
        tiene_csrf = csrf_token is not None or "csrf_token" in input_names
        ...
        if csrf_token:
            logger.info(f"✅ Token CSRF obtenido: {csrf_token[:10]}...")
            login_data["csrf_token"] = csrf_token
        elif tiene_csrf:
            logger.warning(
                "⚠️ El formulario requiere CSRF pero no se pudo obtener el token"
            )
        ...
        # Guardar detalles de la respuesta para diagnóstico
        logger.info(
            f"Respuesta de login: Status {login_response.status_code}, URL final: {login_response.url}"
        )

        try:
            login_response.raise_for_status()
        except requests.HTTPError as e:
            logger.error(f"❌ Error HTTP durante el login: {str(e)}")
            logger.info(f"Headers de respuesta: {dict(login_response.headers)}")
            return False, None
        ...
        if "dashboard" in login_response.url or "admin" in login_response.url:
            ...
            try:
                user_info_response = session.get(
                    f"{BASE_URL}/admin/maintenance/dashboard_user"
                )
...
def verificar_rutas_catalogs(session):
    ...
    rutas = ["/catalogs", "/catalogs/", "/catalogs/create"]
    ...
    for ruta in rutas:
        try:
            ...
            logger.info(
                f"Resultado para {ruta}: {response.status_code} - {'Redirección a ' + response.url if response.url != url else 'Sin redirección'}"
            )

        except Exception as e:
            logger.error(f"Error al verificar ruta {ruta}: {str(e)}")
            resultados[ruta] = {"status_code": None, "error": str(e)}
...
def verificar_catalogo_especifico(session, db):
    ...
    try:
        ...
        logger.info(
            f"Resultado para catálogo {catalog_id}: {response.status_code} - {'Redirección a ' + response.url if response.url != url else 'Sin redirección'}"
        )
...
def verificar_plantillas_catalogs():
    ...
    for plantilla in plantillas:
        ...
        resultado = {"existe": False, "ruta": None, "tamaño": None, "error": None}
        ...
        try:
            ...
            logger.info(
                f"Plantilla {plantilla}: {'✅ Encontrada en ' + resultado['ruta'] if resultado['existe'] else '❌ No encontrada'}"
            )
...
def verificar_blueprints():
    ...
    try:
        ...
        import_match = re.search(
            r"from\s+app\.routes\.catalogs_routes\s+import\s+catalogs_bp", contenido
        )

        # Buscar registro del blueprint
        register_match = re.search(
            r"app\.register_blueprint\(\s*catalogs_bp\s*(?:,\s*url_prefix\s*=\s*['\"](.+)['\"]\s*)?\)",
            contenido,
        )
        ...
        resultado = {
            ...
            "url_prefix": register_match.group(1)
            if register_match and register_match.groups()
            else None,
        }

        logger.info(
            f"Blueprint catalogs_bp: {'✅ Importado' if resultado['importado'] else '❌ No importado'}, {'✅ Registrado' if resultado['registrado'] else '❌ No registrado'}, URL prefix: {resultado['url_prefix'] or 'No especificado'}"
        )
...
def verificar_sesion_usuario(session):
    ...
    try:
            ...
            if script.string and "var userInfo" in script.string:
                ...
                try:
                    user_info_match = re.search(
                        r"var\s+userInfo\s*=\s*({.+?});", script.string, re.DOTALL
                    )
                    if user_info_match:
                    ...
                    logger.warning(
                        f"No se pudo extraer información del usuario del script: {str(e)}"
                    )
        ...
        logger.info(
            f"Información de sesión: Cookie de sesión presente: {session_info['session_cookie_present']}"
        )
...
def verificar_servidor_flask():
    """Verifica si el servidor Flask está en ejecución y accesible."""
    try:
        logger.info(
            f"Verificando si el servidor Flask está en ejecución en {BASE_URL}..."
        )
        response = requests.get(BASE_URL, timeout=5)
        logger.info(f"✅ Servidor Flask accesible (Status: {response.status_code})")

        # Verificar si es realmente un servidor Flask
        server_header = response.headers.get("Server", "")
        if "Werkzeug" in server_header or "Flask" in server_header:
            logger.info(f"✅ Confirmado servidor Flask/Werkzeug: {server_header}")
        else:
            logger.info(
                f"⚠️ Servidor no identificado como Flask/Werkzeug: {server_header}"
            )

        return True
    except requests.RequestException as e:
        logger.error(
            f"❌ No se puede acceder al servidor Flask en {BASE_URL}: {str(e)}"
        )
        logger.info(
            "⚠️ Asegúrate de que el servidor Flask esté ejecutándose con el comando:"
        )
...
def main():
    ...
    # Verificar si el servidor Flask está en ejecución
    if not verificar_servidor_flask():
        logger.error("❌ El servidor Flask no está accesible. Abortando diagnóstico.")
        logger.info("⚠️ Inicia el servidor Flask antes de ejecutar este script.")
        return False
    ...
    try:
        ...
        login_success, session_info = iniciar_sesion(
            session, ADMIN_EMAIL, ADMIN_PASSWORD
        )

        # Si las credenciales principales fallan, intentar con credenciales alternativas
        if not login_success:
            logger.warning(
                f"❌ No se pudo iniciar sesión con las credenciales configuradas: {ADMIN_EMAIL}"
            )
            logger.info("Intentando con credenciales alternativas comunes...")

            alt_success, alt_creds = probar_credenciales_alternativas(session)

            if alt_success:
                login_success = True
                logger.info(
                    f"✅ Inicio de sesión exitoso con credenciales alternativas: {alt_creds['email']}"
                )
                logger.info(
                    "⚠️ Considera actualizar el archivo config_debug.py con estas credenciales"
                )
            else:
                logger.error(
                    "❌ No se pudo iniciar sesión con ninguna credencial. Abortando."
                )
                logger.info("Sugerencias:")
                logger.info(
                    "1. Verifica que el servidor Flask esté ejecutándose correctamente"
                )
                logger.info(
                    "2. Verifica que las credenciales en config_debug.py sean correctas"
                )
                logger.info(
                    "3. Intenta obtener las credenciales correctas del administrador del sistema"
                )
                return False
        ...
        for ruta, resultado in rutas_result.items():
            ...
            redirect_str = (
                f"Redirección a {resultado.get('url_final')}"
                if resultado.get("redirect")
                else "Sin redirección"
            )
            error_str = (
                f"Error: {resultado.get('error_message')}"
            ...
            if resultado.get("error_interno"):
                logger.warning(
                    f"    ⚠️ Se detectó mensaje 'Error interno: ruta no encontrada o mal configurada'"
                )

            if resultado.get("error_permisos"):
                logger.warning(
                    f"    ⚠️ Se detectó mensaje 'No tiene permisos para acceder a esta página'"
                )
        ...
        if catalogo_result:
            ...
            status_str = f"{status} ({'OK' if status == 200 else 'ERROR'})"
            redirect_str = (
                f"Redirección a {catalogo_result.get('url_final')}"
                if catalogo_result.get("redirect")
                else "Sin redirección"
            )
            error_str = (
                f"Error: {catalogo_result.get('error_message')}"
                if catalogo_result.get("error_message")
                else ""
            )

            logger.info(
                f"  - Catálogo {catalog_id}: {status_str}, {redirect_str} {error_str}"
            )
            ...
            logger.info(
                f"    * Número de columnas: {len(catalog_data.get('headers', []))}"
            )
        ...
        for plantilla, resultado in plantillas_result.items():
            status_str = (
                "✅ Encontrada" if resultado.get("existe") else "❌ No encontrada"
            )
            ruta_str = f"en {resultado.get('ruta')}" if resultado.get("ruta") else ""
            tamaño_str = (
                f"({resultado.get('tamaño')} bytes)" if resultado.get("tamaño") else ""
            )
        ...
        if blueprints_result:
            ...
            importado_str = (
                "✅ Importado"
                if blueprints_result.get("importado")
                else "❌ No importado"
            )
            registrado_str = (
                "✅ Registrado"
                if blueprints_result.get("registrado")
                else "❌ No registrado"
            )
            prefix_str = (
                f"con prefix '{blueprints_result.get('url_prefix')}'"
                if blueprints_result.get("url_prefix")
                else "sin prefix específico"
            )

            logger.info(
                f"  - Blueprint catalogs_bp: {importado_str}, {registrado_str}, {prefix_str}"
            )
        ...
        if sesion_result:
            ...
            cookie_str = (
                "✅ Presente"
                if sesion_result.get("session_cookie_present")
                else "❌ Ausente"
            )
        ...
        if ruta_catalogs.get("status_code") != 200 and not ruta_catalogs.get(
            "redirect"
        ):
        ...
        plantillas_faltantes = [
            p for p, r in plantillas_result.items() if not r.get("existe")
        ]
        if plantillas_faltantes:
            problemas.append(
                f"Faltan las siguientes plantillas: {', '.join(plantillas_faltantes)}"
            )
        ...
        if blueprints_result:
            ...
            if not blueprints_result.get("registrado"):
                problemas.append(
                    "El blueprint catalogs_bp no está registrado en app.py"
                )
        ...
        if blueprints_result and (
            not blueprints_result.get("importado")
            or not blueprints_result.get("registrado")
        ):
            logger.info("2. Verificar la configuración del blueprint en app.py:")
            logger.info(
                "   - Asegurarse de que se importa: from app.routes.catalogs_routes import catalogs_bp"
            )
            logger.info(
                "   - Asegurarse de que se registra: app.register_blueprint(catalogs_bp)"
            )

        logger.info(
            "3. Verificar la función check_catalog_permission en app/routes/catalogs_routes.py:"
        )
        logger.info("   - Asegurarse de que maneja correctamente los permisos")
        logger.info(
            "   - Verificar que la sesión contiene la información necesaria (username, email, role)"
        )
