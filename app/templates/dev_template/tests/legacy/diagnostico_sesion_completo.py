...
def crear_app_diagnostico():
    ...
    try:
        ...
        for key in dir(session_config):
            if key.startswith("SESSION_") or key in (
                "SECRET_KEY",
        ...
        logging.warning(
            "⚠️ No se pudo cargar session_config.py, usando configuración predeterminada"
        )
    ...
    if "SESSION_FILE_DIR" in app.config and not os.path.isabs(
        app.config["SESSION_FILE_DIR"]
    ):
        app.config["SESSION_FILE_DIR"] = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), app.config["SESSION_FILE_DIR"]
        )
    ...
    try:
        ...
        logging.warning(
            "⚠️ Flask-Session no está instalado, usando sesiones predeterminadas de Flask"
        )
    except Exception as e:
    ...
    def api_info():
        ...
        info = {
            ...
            "config": {
                ...
                or k in ("SECRET_KEY", "PERMANENT_SESSION_LIFETIME")
            },
            "system": {
