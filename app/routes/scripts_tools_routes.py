...
# Crear el blueprint
scripts_tools_bp = Blueprint(
    "scripts_tools_api", __name__, url_prefix="/admin/scripts-tools-api"
)
...
def list_files_with_info(directory, page=1, per_page=20, filetype=None):
    ...
    for name in sorted(os.listdir(directory)):
        ...
        if (
            filetype
            and not name.endswith(f".{filetype}")
        ...
        # Solo incluir archivos Python (.py) y Shell (.sh), excluir __pycache__ y
        ...
        if not is_dir:
            ...
            if "app/" in rel_path and name in [
                "models.py",
                "extensions.py",
                "decorators.py",
                "error_handlers.py",
        ...
        try:
            ...
            logger.error(
                f"Error al obtener información del archivo {full_path}: {str(e)}"
            )
            size = None
            mtime = None
        ...
        if not is_dir and (name.endswith(".py") or name.endswith(".sh")):
            try:
                ...
                logger.error(
                    f"Error al leer descripción del archivo {full_path}: {str(e)}"
                )
...
def run_logging_unified():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Error en logging unificado: {str(e)}"}
        )
...
def run_clean_logging():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Error en limpieza de logs: {str(e)}"}
        )
...
def run_config_embedded():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Error en configuración embebida: {str(e)}"}
        )
...
def run_monitoring():
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Error en monitoreo: {str(e)}"})
...
def run_notifications():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Error en notificaciones: {str(e)}"}
        )
...
def run_data_fallback():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Error en datos de respaldo: {str(e)}"}
        )
...
def run_maintenance_utility(utility):
    ...
    try:
        if utility == "normalize_users":
            ...
            return (
                jsonify(
                    {"error": f"Utilidad de mantenimiento no reconocida: {utility}"}
                ),
                400,
            )
...
def run_normalize_users():
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Error en normalización: {str(e)}"})
...
def run_backup_users():
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Error en backup: {str(e)}"})
...
def run_create_profile_image():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Error en imagen de perfil: {str(e)}"}
        )
...
def run_excel_utils():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Error en utilidades Excel: {str(e)}"}
        )
...
def run_cleanup_project():
    ...
    try:
        ...
        if os.path.exists(cleanup_script):
            ...
            return jsonify(
                {"success": False, "error": "Script de limpieza no encontrado"}
            )
    except Exception as e:
        return jsonify({"success": False, "error": f"Error en limpieza: {str(e)}"})
...
def run_data_utility(utility):
    ...
    try:
        if utility == "create_backup":
            ...
            return (
                jsonify({"error": f"Utilidad de datos no reconocida: {utility}"}),
                400,
            )
...
def run_restore_backup():
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Error en restauración: {str(e)}"})
...
def run_validate_integrity():
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Error en validación: {str(e)}"})
...
def run_image_utils():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Error en utilidades de imagen: {str(e)}"}
        )
...
def run_spreadsheet_utils():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Error en utilidades de spreadsheet: {str(e)}"}
        )
...
def run_s3_utils():
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Error en utilidades S3: {str(e)}"})
