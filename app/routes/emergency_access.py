...
# Descripción: [Rutas de acceso de emergencia que establecen la sesión directamente sin depender del sistema normal de inicio de sesión.]
...
def user_login_bypass():
    ...
    try:
        ...
        logger.warning(
            f"✅ ACCESO DIRECTO: Sesión establecida para usuario normal: {session['email']}"
        )
        flash(
            "Has iniciado sesión como usuario normal mediante acceso directo", "success"
        )
        return redirect("/")
...
def admin_login_bypass():
    ...
    try:
        ...
        session.modified = True
        logger.warning(
            f"✅ ACCESO DIRECTO: Sesión establecida para administrador: {session['email']}"
        )
        flash(
            "Has iniciado sesión como administrador mediante acceso directo", "success"
        )
