import os
from flask import Blueprint, send_file, render_template_string, abort, request, jsonify, session, current_app
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if flask_login_available and current_user is not None:
            ...
            if hasattr(current_user, 'is_authenticated') and hasattr(current_user, 'is_admin'):
                logger.debug(f"[admin_required] is_authenticated: {current_user.is_authenticated}")
                logger.debug(f"[admin_required] is_admin: {getattr(current_user, 'is_admin', None)}")
                if not current_user.is_authenticated or not current_user.is_admin:
                    logger.warning("[admin_required] Bloqueado por Flask-Login (no admin o no autenticado)")
                    return jsonify({'success': False, 'error': 'Solo administradores'}), 403
        # 2. Fallback: session
        elif session.get('username') and session.get('role') == 'admin':
            logger.debug(f"[admin_required] session: {dict(session)} (admin OK por session)")
...
README_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../dev_template/README.md'))
...
TESTS_BASE = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../dev_template/tests'))
INTEGRATION_DIR = os.path.join(TESTS_BASE, 'integration')
APP_ROUTES_DIR = os.path.join(TESTS_BASE, 'app', 'routes')
PYTEST_REPORT = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../dev_template/pytest_report.html'))
...
def update_tests_readme():
    ...
    """
    import re
    TESTS_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../dev_template/tests'))
    README_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../tools/README.md'))
    ...
    try:
        with open(README_PATH, 'r', encoding='utf-8') as f:
            content = f.read()
        logger.info(f"[update-tests-readme] README.md leído correctamente: {README_PATH}")
        new_section = f'{START}\n{table}\n{END}'
        # Verificar que ambos delimitadores existen
        if START not in content or END not in content:
            logger.error(f"[update-tests-readme] Delimitadores no encontrados en README. START: {START in content}, END: {END in content}")
            return jsonify({'success': False, 'error': 'No se encontraron los delimitadores de sección automática en README.md'}), 400
        # Reemplazo seguro
        updated, n = re.subn(f'{START}.*?{END}', new_section, content, flags=re.DOTALL)
        if n == 0:
            logger.error("[update-tests-readme] El bloque delimitado no fue reemplazado. ¿Delimitadores corruptos?")
            return jsonify({'success': False, 'error': 'No se pudo reemplazar el bloque delimitado en README.md'}), 400
...
def script_params_help():
    ...
    script_abs = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../dev_template/tests', rel_path))
    TESTS_BASE = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../dev_template/tests'))
    ...
    ayuda = ''
    try:
        proc = subprocess.run(['python3', script_abs, '--help'], capture_output=True, text=True, timeout=4)
        ...
        for f in files:
            if f.endswith('.py') and not f.startswith('__'):
                ...
                try:
                    ...
                    desc = ''
                scripts_info.append({'name': f, 'desc': desc, 'path': (rel_dir + '/' + f) if rel_dir != 'varios' else f})
        if scripts_info:
...
def generate_test_template():
    ...
    if not name:
        return jsonify(success=False, error='Falta el nombre del modelo o endpoint.'), 400
    code = ''
    if name.startswith('/'):
        # Asume endpoint REST
        endpoint = name
...
def run_test():
    ...
    if not _test_lock.acquire(blocking=False):
        return jsonify({'result': 'Ya hay una ejecución de tests en curso. Espera a que termine.'})
    try:
        data = request.get_json(force=True)
...
def download_report():
    ...
    return send_from_directory(os.path.dirname(PYTEST_REPORT), os.path.basename(PYTEST_REPORT), as_attachment=True)
