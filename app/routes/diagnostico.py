...
from flask import Blueprint, jsonify, session, request, current_app, render_template, flash, redirect, url_for
...
def diagnostico_session():
    ...
    logger.info(f"Diagnóstico de sesión solicitado: {json.dumps(session_info, default=str)}")
...
def login_directo():
    ...
    try:
        ...
        session['username'] = usuario.get('username') or usuario.get('nombre') or 'admin'
        session['role'] = 'admin'
        ...
        # Registrar información para análisis
        logger.info(f"Acceso directo establecido para usuario: {session.get('username')} (ID: {session.get('user_id')})")
...
def fix_session():
    ...
    try:
        ...
        logger.info(f"Sesión reparada para usuario: {session.get('username')} (ID: {session.get('user_id')})")
