...
def admin_login_direct():
    ...
    try:
        ...
        logger.warning(
            f"Entorno detectado: {'LOCAL' if is_local else 'PRODUCCIÓN'} | Host: {request.host}"
        )
        ...
        if not admin_user and not is_local:
            try:
                ...
                if not admin_user:
                    ...
                    if admin_user:
                        logger.info("✅ Usuario admin encontrado en users_unified")
                    else:
                        # En users
                        try:
                            ...
                            admin_user = users_collection.find_one({"role": "admin"})
                            if admin_user:
                                logger.info("✅ Usuario admin encontrado en users")
                        except Exception as coll_err:
                            logger.warning(
                                f"Error accediendo a colección users: {coll_err}"
                            )
        ...
        if not admin_user:
            ...
            flash(
                "No se encontraron usuarios administradores en la base de datos. Se ha creado uno temporal.",
                "warning",
            )

        # 4. ESTABLECER SESIÓN CON GARANTÍAS TOTALES
        ...
        flash(
            f"Se ha detectado un error, pero se ha establecido una sesión de administrador de emergencia. Error: {str(e)}",
            "warning",
        )
