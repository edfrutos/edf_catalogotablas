            ...
            try:
                ...
                for line in lines:
                    ...
                    if any(
                        ...
                        for keyword in ["error", "warning", "critical", "exception"]
                    ):
                        filtered_lines.append(line)
                    # Mantener líneas de inicio/fin de sesión
                    elif any(
                        keyword in line.lower()
                        for keyword in ["login", "logout", "session"]
                    ):
                        filtered_lines.append(line)
                    # Mantener líneas de configuración
                    elif any(
                        keyword in line.lower()
                        for keyword in ["config", "setup", "init"]
                    ):
                        filtered_lines.append(line)
                ...
                if size_before != size_after:
                    cleaned_count += 1
                    print(
                        f"   ✅ {os.path.basename(log_file)}: {size_before/1024:.1f}KB → {size_after/1024:.1f}KB"
                    )
