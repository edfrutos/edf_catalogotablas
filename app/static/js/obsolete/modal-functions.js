...
// Función para mostrar PDF en modal - VISUALIZACIÓN LOCAL
function showPDFInModal(pdfUrl, pdfTitle) {
    ...
    if (buttonGroup && pdfContent) {
        ...
        // VISUALIZACIÓN LOCAL: Usar PDF.js para renderizar PDF
        console.log('[PDF] 🚀 Iniciando visualización local del PDF con PDF.js...');
        ...
        // Limpiar contenido anterior y agregar canvas
        pdfContent.innerHTML = '';
        pdfContent.appendChild(pdfCanvas);
        
        console.log('[PDF] 🔧 Contenido del modal después de agregar canvas:', pdfContent.innerHTML);
        
        // Cargar PDF usando PDF.js
        if (pdfUrl.startsWith('/uploads/')) {
            const localUrl = `/static/uploads/documents/${pdfUrl.split('/').pop()}`;
            console.log('[PDF] 🔧 PDF local cargado:', localUrl);
            
                    // Verificar que PDF.js esté disponible
        if (typeof pdfjsLib === 'undefined') {
            console.error('[PDF] ❌ PDF.js no está disponible');
            pdfContent.innerHTML = `
        ...
        console.log('[PDF] 🔧 PDF.js disponible:', typeof pdfjsLib);
        console.log('[PDF] 🔧 Versión PDF.js:', pdfjsLib.version);
        
        // Usar PDF.js para renderizar
        pdfjsLib.getDocument(localUrl).promise.then(function(pdf) {
            console.log('[PDF] ✅ PDF cargado correctamente, páginas:', pdf.numPages);
            ...
            pdf.getPage(1).then(function(page) {
                ...
                page.render(renderContext).promise.then(function() {
                    console.log('[PDF] ✅ Primera página renderizada correctamente');
                });
            });
        }).catch(function(error) {
            console.error('[PDF] ❌ Error cargando PDF:', error);
            ...
            console.log('[PDF] 🔧 PDF externo cargado:', pdfUrl);
        }
        
        console.log('[PDF] 🔧 Canvas creado con dimensiones:', {
        ...
        setTimeout(() => {
            ...
            console.log('- canvas visible:', pdfCanvas.style.display);
        }, 1000);
        
    } else {
        console.error('[PDF] ❌ Elementos del DOM no encontrados');
    }
}
