...
$(function() {
  ...
  $('#logFileSelect').on('change', function() {
    ...
    $.get(`/admin/maintenance/logs/view?file=${encodeURIComponent(logFile)}`, function(data) {
      if (data && data.status === 'success') {
        $('#logPreview').text(data.content || 'El archivo de log está vacío');
      } else if (data && data.message) {
        $('#logPreview').text(`Error al cargar el archivo de log: ${data.message}`);
      } else {
        $('#logPreview').text('Error desconocido al cargar el archivo de log.');
      }
    }).fail(function() {
    ...
    if (useDateRange) {
      if (!startDate || !endDate) {
        $('#cleanupAlert')
          .removeClass('alert-success')
          .addClass('alert-danger')
          .html(`<h5><i class="bi bi-exclamation-triangle-fill"></i> Debes indicar fecha/hora de inicio y fin.</h5>`);
        return;
      }
      ...
      if (!days || isNaN(days) || days <= 0) {
        $('#cleanupAlert')
          .removeClass('alert-success')
          .addClass('alert-danger')
          .html(`<h5><i class="bi bi-exclamation-triangle-fill"></i> Debes indicar un número de días válido.</h5>`);
        return;
      }
    ...
    $.get('/admin/maintenance/api/scheduled_tasks', function(data) {
      if (data && Array.isArray(data.tasks)) {
        data.tasks.forEach(function(task) {
          const row = $(`#scheduledTasks button[data-task="${task.id}"]`).closest('tr');
          row.find('.badge').removeClass('bg-secondary bg-success bg-danger').addClass(task.estado === 'OK' ? 'bg-success' : (task.estado === 'ERROR' ? 'bg-danger' : 'bg-secondary')).text(task.estado);
  ...
  $('#scheduledTasks').on('click', '.run-task', function() {
    ...
    btn.html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Ejecutando...');
    ...
    $.post('/admin/maintenance/api/run_task', { task: task }, function(data) {
      ...
      $('.run-task').prop('disabled', false).html('<i class="bi bi-play"></i> Ejecutar Ahora');
      $alert.show()
        .removeClass('alert-success alert-danger')
        .addClass(data.status === 'success' ? 'alert-success' : 'alert-danger')
        .html(`<strong>${data.status === 'success' ? 'Éxito' : 'Error'}:</strong> ${data.message}`)
        .focus();
      loadScheduledTasks();
    }).fail(function(xhr) {
      $('.run-task').prop('disabled', false).html('<i class="bi bi-play"></i> Ejecutar Ahora');
      $alert.show()
        ...
        .html('<strong>Error:</strong> No se pudo ejecutar la tarea.')
        .focus();
    });
  });
  ...
  function loadSystemStatus() {
    $.get('/admin/maintenance/api/system_status', function(data) {
      ...
      $('#systemStatus').find('span,div.progress-bar').text('Error de conexión');
    });
  }
  ...
  function exportSystemStatus() {
    ...
    // Convierte a CSV
    const csvContent = data.map(row => row.map(cell => '"' + cell.replaceAll('"', '""') + '"').join(',')).join('\n');
