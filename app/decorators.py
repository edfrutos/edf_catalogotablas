...
def login_required(f: F) -> F:
    ...
    def decorated_function(*args: Any, **kwargs: Any) -> ResponseReturnValue:
        logger.info(
            f"[LOGIN_REQUIRED] Ruta: {request.path}, Método: {request.method}, Sesión actual: {dict(session)}"
        )
        # Verificar tanto user_id como logged_in para mayor seguridad
        if "user_id" not in session or not session.get("logged_in", False):
            logger.warning(
                f"[LOGIN_REQUIRED] Autenticación fallida. user_id: {session.get('user_id')}, logged_in: {session.get('logged_in')}"
            )
        ...
        logger.debug(
            f"[LOGIN_REQUIRED] Acceso permitido a {request.path} para usuario {session.get('username')} (ID: {session.get('user_id')})"
        )
...
def admin_required(f: F) -> F:
    ...
    def decorated_function(*args: Any, **kwargs: Any) -> ResponseReturnValue:
        logger.info(
            f"[ADMIN_REQUIRED] Ruta: {request.path}, Método: {request.method}, Sesión actual: {dict(session)}"
        )
        ...
        if not session.get("logged_in", False) or "user_id" not in session:
            logger.warning(
                f"[ADMIN_REQUIRED] Usuario no autenticado intentando acceder a ruta admin: {request.path}"
            )
        ...
        if session.get("role") != "admin":
            logger.warning(
                f"[ADMIN_REQUIRED] Usuario sin permisos de admin intentando acceder: {session.get('username')} (role: {session.get('role')})"
            )
            flash("No tienes permisos para acceder a esta sección.", "danger")
            return redirect(url_for("main.dashboard_user"))
        # Registrar acceso exitoso a ruta admin
        logger.debug(
            f"[ADMIN_REQUIRED] Acceso admin permitido a {request.path} para usuario {session.get('username')}"
        )
        return f(*args, **kwargs)
