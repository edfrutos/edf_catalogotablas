...
def get_user_collection():
    ...
    try:
        ...
        if hasattr(current_app, 'users_collection'):
            return current_app.users_collection
...
def create_user(username: str, email: str, password: str, role: str = 'user') -> Optional[str]:
    ...
    try:
        ...
        if collection.find_one({"$or": [{"username": username}, {"email": email}]}):
            logger.warning(f"Ya existe un usuario con el nombre {username} o email {email}")
            return None
...
def change_password(user_id: str, current_password: str, new_password: str) -> bool:
    ...
    try:
        ...
        if not verify_password(user, current_password):
            logger.warning(f"Contraseña actual incorrecta para el usuario {user.get('username')}")
            return False
