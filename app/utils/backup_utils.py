...
class FileProcessor:
    ...
    def cleanup_temp_files(self):
        ...
        for temp_file in self.temp_files:
            try:
                ...
                log_warning(f"No se pudo eliminar archivo temporal {temp_file}: {str(e)}")
    ...
    def validate_file(self, filename: str, content: bytes) -> bool:
        ...
        if file_path.suffix.lower() not in self.SUPPORTED_EXTENSIONS:
            if not (file_path.suffixes and 
                   ''.join(file_path.suffixes[-2:]).lower() in self.SUPPORTED_EXTENSIONS):
                raise FileProcessingError(f"Extensión de archivo no soportada: {filename}")
    ...
    def process_file_content(self, content: bytes, filename: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for row in csv_reader:
                ...
                collection_name = row.get('collection', 
                                        row.get('table', 
                                              row.get('coleccion', 'imported_data')))
            ...
            log_info(f"CSV procesado: {row_count} filas en {len(collections)} colecciones")
...
class BackupManager:
    ...
    def create_backup(self, collections: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if collections:
                ...
                collection_names = [
                    name for name in self.db.list_collection_names()
            ...
            for collection_name in collection_names:
                try:
                    ...
                    log_info(f"Colección {collection_name}: {len(processed_docs)} documentos")
                except Exception as e:
                    log_error(f"Error respaldando colección {collection_name}: {str(e)}")
                    raise BackupError(f"Error respaldando colección {collection_name}: {str(e)}")
            
            backup_data['metadata']['total_collections'] = len(collection_names)
            backup_data['metadata']['total_documents'] = total_documents
            
            log_info(f"Backup creado: {len(collection_names)} colecciones, {total_documents} documentos")
            return backup_data
        except Exception as e:
    ...
    def _process_document_for_backup(self, doc: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in doc.items():
            if isinstance(value, ObjectId):
                ...
                processed[key] = [self._process_document_for_backup(item) if isinstance(item, dict) else item for item in value]
            else:
                processed[key] = value
        return processed
    
    def restore_backup(self, backup_data: Dict[str, Any], overwrite: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            for collection_name, documents in collections.items():
                try:
                    ...
                    result = self._restore_collection(collection_name, documents, overwrite)
            ...
            log_info(f"Restauración completada: {results['total_documents']} documentos insertados, {results['skipped_documents']} omitidos")
            return results
        except Exception as e:
    ...
    def _restore_collection(self, collection_name: str, documents: List[Dict[str, Any]], overwrite: bool = False) -> Dict[str, Any]:
        ...
        if overwrite:
            try:
                ...
                log_warning(f"No se pudo limpiar la colección {collection_name}: {str(e)}")
    ...
    def _process_document_for_restore(self, doc: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in doc.items():
            if key == '_id' and isinstance(value, str):
                ...
                processed[key] = [self._process_document_for_restore(item) if isinstance(item, dict) else item for item in value]
            else:
                processed[key] = value
        return processed
...
class GoogleDriveManager:
        ...
        try:
            if not self.storage_client:
                ...
                if self.storage_client:
                    ...
                    raise GoogleDriveError("No se pudo inicializar el cliente de Google Drive")
    ...
    def list_backups(self, folder_name: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not folder_id:
                log_warning(f"No se pudo acceder a la carpeta {folder_name}")
                return []
            ...
            for file_info in files:
                if file_info['title'].endswith('.json.gz') or file_info['title'].endswith('.json'):
                    backups.append({
                        'id': file_info['id'],
                        'name': file_info['title'],
                        'size': file_info.get('fileSize', 0),
                        'created_date': file_info.get('createdDate', ''),
                        'modified_date': file_info.get('modifiedDate', ''),
    ...
    def upload_file(self, content: bytes, filename: str, folder_name: str = None) -> str:
        ...
        try:
            ...
            folder_name = folder_name or self.folder_name
            log_info(f"Subiendo archivo: {filename} ({len(content)} bytes) a carpeta: {folder_name}")
