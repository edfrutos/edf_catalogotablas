...
def get_referenced_images(db):
    ...
    for collection_name in db.list_collection_names():
        ...
        if collection_name == catalog_collection_name:
            logger.info(
                f"Procesando colección principal de catálogo: {collection_name}"
            )
        ...
        for doc in collection.find({}, {"Imagenes": 1}):
            if "Imagenes" in doc and doc["Imagenes"]:
                ...
                if collection_name == catalog_collection_name:
                    logger.debug(
                        f"Encontrado documento con imágenes en catálogo: {doc['_id']}"
                    )
...
def get_local_images():
    ...
    logger.info(
        f"Total de archivos encontrados en {Config.UPLOAD_FOLDER}: {len(local_images)}"
    )
    return local_images
...
def clean_unused_images(referenced_images, local_images):
    ...
    if Config.CLEANUP_MODE == "move" and not os.path.exists(
        Config.UNUSED_IMAGES_FOLDER
    ):
    ...
    # Estadísticas
    stats = {"moved": 0, "deleted": 0, "errors": 0, "details": []}
...
def send_email_summary(stats, referenced_images, local_images):
    ...
    try:
        ...
        msg["Subject"] = (
            f"Reporte de limpieza de imágenes - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}"
        )
...
def main():
    ...
    logger.info(
        f"Imágenes no utilizadas: {max(0, len(local_images) - len(referenced_images))}"
    )
