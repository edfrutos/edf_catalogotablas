...
# Buscar importaciones de tools en todos los .py del proyecto, excluyendo _backup_scripts
IMPORTS=$(grep -rE "from tools\\.|import tools\\." "$ROOT_DIR" --include='*.py' | grep -v 'venv' | grep -v 'site-packages' | grep -v '_backup_scripts')

if [ -z "$IMPORTS" ]; then
  echo "No se encontraron importaciones de scripts de tools en la app principal."
  exit 0
fi

echo "==== SCRIPTS DE /tools REQUERIDOS POR LA APP PRINCIPAL ===="
...
echo "\nIMPORTACIONES DETECTADAS EN LA APP PRINCIPAL:"
echo "$IMPORTS" 