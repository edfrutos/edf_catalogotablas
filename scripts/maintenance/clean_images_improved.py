...
class ImageCleaner:
    ...
    def __init__(self, config: dict = None):
        ...
        os.makedirs(self.config['unused_dir'], exist_ok=True)
    ...
    def is_image_used(self, image_path: str) -> bool:
        ...
        try:
            ...
            for collection, fields in collections_to_check:
                ...
                query["$or"].append({"image_hash": file_hash})
    ...
    def is_valid_image(self, file_path: str) -> bool:
        ...
        try:
            with Image.open(file_path) as img:
                ...
                if min(img.size) < self.config['min_dimension']:
                    self.logger.warning(f"Imagen muy pequeña: {file_path} ({img.size[0]}x{img.size[1]})")
                    return False
                ...
                for img_path in Path(img_dir).rglob(f'*{ext}'):
                    ...
                    try:
                        with Image.open(img_path) as img:
                            ...
                            img_hash = str(imagehash.average_hash(img, self.config['hash_size']))
    ...
    def process_file(self, filepath: str, duplicates: Dict[str, List[str]], dry_run: bool = True) -> None:
        ...
        try:
            ...
            if not (self.config['min_file_size'] <= file_stat.st_size <= self.config['max_file_size']):
                self.logger.warning(f"Tamaño de archivo fuera de rango: {filepath} ({file_stat.st_size} bytes)")
                return
            ...
            is_duplicate = any(
                ...
                if len(dup_list) > 1 and filepath != dup_list[0]  # Mantener la primera ocurrencia
            )
            ...
            elif file_age > self.config['min_age_days'] and not self.is_image_used(filepath):
            ...
            if dry_run:
                ...
                try:
                    ...
                    backup_path = os.path.join(self.config['unused_dir'], os.path.basename(filepath))
            ...
            with ThreadPoolExecutor(max_workers=self.config['max_workers']) as executor:
                for root, _, files in os.walk(img_dir):
                    ...
                    for filename in files:
                        if not any(filename.lower().endswith(ext) for ext in self.config['extensions']):
                            continue
                        ...
                        # Procesar archivo
                        executor.submit(self.process_file, filepath, duplicates, dry_run)
    ...
    def _print_summary(self, dry_run: bool) -> None:
        ...
        if dry_run:
            ...
            summary.append(f"\nLas imágenes se movieron a: {self.config['unused_dir']}")
...
def parse_arguments():
    ...
    parser.add_argument('--min-age', type=int, help='Edad mínima en días para considerar una imagen como no utilizada')
