#!/usr/bin/env python3
# Script para organizar los scripts que están en el directorio /tools/root
# Creado: 18/05/2025
...
ROOT_SCRIPTS_DIR = os.path.join(TOOLS_DIR, 'root')
...
def organize_scripts():
    """Organiza los scripts que están en el directorio /tools/root"""
    print("Organizando scripts en el directorio /tools/root...")
    
    # Definir categorías y patrones de nombres de archivos
    categories = {
        'admin_utils': ['admin', 'user', 'permission', 'role', 'unlock', 'migrate_users'],
        'db_utils': ['db', 'mongo', 'database', 'collection', 'catalog', 'connect', 'excel', 'verificar'],
        ...
        'catalog_utils': ['catalog', 'tabla', 'rutas']
    }
    ...
    # Listar todos los scripts en el directorio /tools/root
    if not os.path.exists(ROOT_SCRIPTS_DIR):
        print(f"❌ El directorio {ROOT_SCRIPTS_DIR} no existe")
        return 0
    
    scripts_moved = 0
    for item in os.listdir(ROOT_SCRIPTS_DIR):
        source_item = os.path.join(ROOT_SCRIPTS_DIR, item)
    ...
    print(f"Se categorizaron {scripts_moved} scripts del directorio /tools/root")
...
def main():
    print_header("ORGANIZACIÓN DE SCRIPTS EN EL DIRECTORIO /TOOLS/ROOT")
    print(f"Fecha y hora: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Organizar scripts
    scripts_moved = organize_scripts()
    
    # Ejecutar fix_script_paths.py si se movieron scripts
    if scripts_moved > 0:
    ...
    print(f"Se han categorizado {scripts_moved} scripts del directorio /tools/root")
