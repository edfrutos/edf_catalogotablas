...
ROUTES_FILE = os.path.join(ROOT_DIR, "local/app/routes/scripts_routes.py")
...
def create_script_symlinks():
    ...
    for script_path, rel_path, file in scripts_found:
        ...
        if os.path.exists(symlink_path) and not os.path.islink(symlink_path):
            print(
                f"  ⚠️ Ya existe un archivo con el nombre {file}. No se creará enlace."
            )
            continue
...
def improve_get_script_path():
    ...
    try:
        ...
        get_script_path_match = re.search(
            r"def get_script_path\([^)]*\):(.*?)def", content, re.DOTALL
        )
...
"""
        ...
        new_content = content.replace(
            get_script_path_match.group(0), improved_function + "\ndef "
        )
...
def fix_run_script_route():
    ...
    if not os.path.exists(
        f"{ROUTES_FILE}.bak.{datetime.datetime.now().strftime('%Y%m%d')}"
    ):
        backup_file(ROUTES_FILE)
    ...
    try:
        ...
        run_script_route_match = re.search(
        ...
        if not run_script_route_match:
            ...
            print(
                f"ℹ️ Ya existe una ruta para ejecutar scripts: {run_script_route_match.group(1)}"
            )
            return True
    except Exception as e:
        print(f"❌ Error al modificar scripts_routes.py: {str(e)}")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if os.path.exists(restart_script):
                ...
                if result.returncode == 0:
                    print(
                        "✅ Servidor Gunicorn reiniciado correctamente mediante script"
                    )
                    return True
                else:
                    print(
...
def main():
    ...
    print(
        "2. Se mejoró la función get_script_path para buscar scripts en más ubicaciones"
    )
