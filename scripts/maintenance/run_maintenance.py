...
def setup_logging():
    ...
    log_file = MAINTENANCE_LOG_DIR / f"maintenance_{datetime.now().strftime('%Y%m%d')}.log"
...
def check_mongodb_connection(logger):
    ...
    try:
        ...
        try:
            ...
            if not mongo_uri:
                logger.error("❌ No se pudo encontrar MONGO_URI en config.py ni en variables de entorno")
                return False
...
def main():
    parser = argparse.ArgumentParser(description='Script de mantenimiento para EDF Catálogo Tablas')
    parser.add_argument('--days', type=int, default=30, help='Días de retención para logs')
    parser.add_argument('--start-datetime', type=str, help='Fecha/hora inicio (ISO 8601) para limpieza de logs')
    parser.add_argument('--end-datetime', type=str, help='Fecha/hora fin (ISO 8601) para limpieza de logs')
    parser.add_argument('--task', choices=['all', 'logs', 'disk', 'mongo'], default='all',
    ...
    try:
        if args.task in ['all', 'logs']:
            ...
            if args.start_datetime and args.end_datetime:
                ...
                cleaner = LogCleaner(log_dir=str(LOG_DIR), retention_days=None, dry_run=False,
