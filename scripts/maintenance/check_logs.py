...
def analyze_log_file(log_path, max_lines=1000, search_term=None):
    ...
    if not os.path.exists(log_path):
        return {
            "success": False,
            "error": f"El archivo de log no existe: {log_path}"
        }
    
    try:
        # Patrones para detectar errores comunes
        error_patterns = {
            "script_not_found": re.compile(r"Script no encontrado|No such file or directory|Cannot find|404"),
            ...
            "execution_error": re.compile(r"Error al ejecutar|execution failed|failed to execute")
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Analiza logs para detectar problemas de ejecución de scripts')
    parser.add_argument('--log', help='Ruta al archivo de log a analizar', default=None)
    parser.add_argument('--lines', type=int, help='Número máximo de líneas a analizar', default=1000)
    parser.add_argument('--search', help='Término de búsqueda para filtrar líneas', default=None)
            ...
            if not result["error_counts"]:
                ...
                for error_type, examples in result["error_examples"].items():
                    ...
                    for i, example in enumerate(examples, 1):
                        print(f"    {i}. {example[:150]}..." if len(example) > 150 else f"    {i}. {example}")
        else:
            print(f"Error: {result['error']}")
    
    print_header("RECOMENDACIONES")
    print("1. Si encuentra errores 'script_not_found', verifique las rutas de los scripts")
    print("2. Para errores de 'permission_denied', ejecute el script fix_script_permissions.sh")
    print("3. Para errores de 'import_error', verifique las dependencias del script")
    print("4. Si hay errores de 'timeout', considere aumentar el tiempo de espera en script_runner.py")
