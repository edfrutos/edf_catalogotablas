...
def restore_collection():
    json_file = input('Introduce el nombre del archivo .json de backup a restaurar (en backups/): ').strip()
    ...
    print(f'✔ Restaurados/actualizados {n_upserted} documentos en la colección {collection_name}')
...
def restore_multiple_collections():
    ...
    for i, f in enumerate(files):
        print(f'{i+1}. {f}')
    indices = input('Introduce los números de los archivos a restaurar separados por coma (ej: 1,3,5): ').strip()
    ...
    for idx in indices:
        if 0 <= idx < len(files):
            ...
            for d in docs:
                ...
                result = db[collection_name].replace_one({'_id': d.get('_id')}, d, upsert=True)
                n_upserted += result.upserted_id is not None or result.modified_count > 0
            print(f'✔ Restaurados/actualizados {n_upserted} documentos en {collection_name} desde {json_file}')
