...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("clean_images.log"), logging.StreamHandler()],
)
...
class ImageCleaner:
    def __init__(self, config_file="clean_images_config.json"):
        ...
        self.extensions = self.config.get(
    ...
    def _load_config(self, config_file):
        ...
        try:
            ...
            logger.error(
                f"Error al decodificar el archivo de configuración {config_file}"
            )
            return {}
            ...
            for img_path in img_paths[1:]:  # Todas menos la más reciente
                if not self.is_image_used(img_path):
                    try:
                        ...
                        logger.info(
                            f"Eliminada imagen duplicada: {img_path} (hash: {img_hash[:8]}...)"
                        )
                        deleted += 1
