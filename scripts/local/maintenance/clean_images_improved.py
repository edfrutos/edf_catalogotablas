                ...
                for img_path in Path(img_dir).rglob(f"*{ext}"):
                    ...
                    try:
                        with Image.open(img_path) as img:
                            ...
                            img_hash = str(
                                imagehash.average_hash(img, self.config["hash_size"])
                            )
    ...
    def process_file(
        ...
        try:
            ...
            is_duplicate = any(
                ...
                if len(dup_list) > 1 and filepath != dup_list[0]
            )
            ...
            elif file_age > self.config["min_age_days"] and not self.is_image_used(
                filepath
            ):
            ...
            if dry_run:
                ...
                try:
                    ...
                    backup_path = os.path.join(
                        self.config["unused_dir"], os.path.basename(filepath)
                    )
