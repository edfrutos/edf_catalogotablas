...
def main():
    ...
    # Conectar a MongoDB
    client = MongoClient(
        MONGO_URI, tls=True, tlsCAFile=certifi.where(), retryWrites=True, w="majority"
    )
    ...
    for table in tables:
        ...
        for record in records:
            # Crear un diccionario con los valores de los campos (excluyendo campos de
            # sistema)
            values = {
                k: v
            ...
            if row_hash in unique_records:
                ...
                catalog_collection.update_one(
                    {"_id": record["_id"]}, {"$set": {"row_hash": row_hash}}
                )
        ...
        remaining = list(
            catalog_collection.find({"table": table}).sort("created_at", 1)
        )
        for idx, record in enumerate(remaining, 1):
            catalog_collection.update_one(
                {"_id": record["_id"]}, {"$set": {"Número": idx}}
            )
