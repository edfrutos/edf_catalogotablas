...
class LogCleaner:
    ...
    def clean_old_logs(self):
        ...
        if self.start_datetime and self.end_datetime:
            self.logger.info(
                f"Eliminando logs entre {self.start_datetime} y {self.end_datetime}"
            )
        elif self.retention_days is not None:
            self.logger.info(
                f"Eliminando logs con más de {self.retention_days} días de antigüedad"
            )
        else:
                ...
                for log_file in self.log_dir.glob(pattern):
                    ...
                    try:
                        ...
                        if borrar:
                            if self.dry_run:
                                self.logger.info(
                                    f"[DRY RUN] Se eliminaría: {log_file} (Última modificación: {datetime.fromtimestamp(file_mtime)})"
                                )
                            else:
                                log_file.unlink()
                                deleted_files += 1
                                ...
                                self.logger.info(
                                    f"Eliminado: {log_file} (Última modificación: {datetime.fromtimestamp(file_mtime)})"
                                )
                    except Exception as e:
                        self.logger.error(f"Error al procesar {log_file}: {str(e)}")
            # Resumen
            self.logger.info(
                f"Proceso completado. Archivos eliminados: {deleted_files}, Espacio liberado: {self._format_size(total_size)}"
            )
            return True
        except Exception as e:
...
def main():
    ...
    try:
        cleaner = LogCleaner(
            log_dir=args.log_dir, retention_days=args.days, dry_run=args.dry_run
        )
