...
def analyze_project_imports() -> Set[str]:
    ...
    exclude_dirs = {'.venv', 'venv', '.venv310', '__pycache__'}
    python_files = [
...
def check_python310_compatibility(package: str, version: str) -> bool:
    ...
    problematic_packages = {
        'mod-wsgi': 'Requiere Apache headers para compilar'
    }
...
def create_clean_requirements(required_packages: Set[str], project_imports: Set[str]):
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        f.write("# =====================================\n")
        google_packages = ['google-api-python-client', 'google-auth-oauthlib', 'google-auth', 'googleapis-common-protos']
        ...
        f.write("# =====================================\n")
        util_packages = ['python-dotenv', 'Pillow', 'click', 'itsdangerous', 'Jinja2', 'MarkupSafe', 'psutil']
    ...
    print(f"📦 Paquetes incluidos: {len([p for p in optimized_versions.keys() if any(p in line for line in open(filename).readlines())])}")
