...
try:
    ...
    from dev_template.tests.legacy.config_debug import BASE_URL, ADMIN_EMAIL, ADMIN_PASSWORD, MONGO_URI
    ...
    logger.warning("⚠️ No se pudo cargar config_debug.py, usando variables de entorno o valores por defecto")
...
def probar_credenciales_alternativas(session):
    ...
    for creds in credenciales_comunes:
        ...
        if success:
            logger.info(f"✅ Inicio de sesión exitoso con credenciales alternativas: {creds['email']}")
            return True, creds
...
def iniciar_sesion(session, email, password):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ Servidor accesible en {BASE_URL} (Status: {server_check.status_code})")
        except requests.RequestException as e:
            logger.error(f"❌ No se puede acceder al servidor en {BASE_URL}: {str(e)}")
            logger.info("⚠️ Asegúrate de que el servidor Flask esté ejecutándose en el puerto correcto")
            return False, None
        ...
        logger.info(f"Formulario encontrado con action: {form.get('action', 'No especificado')}")
        ...
        if csrf_token:
            ...
            logger.warning("⚠️ El formulario requiere CSRF pero no se pudo obtener el token")
        ...
        logger.info(f"Respuesta de login: Status {login_response.status_code}, URL final: {login_response.url}")
...
def verificar_servidor_flask():
    ...
    try:
        logger.info(f"Verificando si el servidor Flask está en ejecución en {BASE_URL}...")
        ...
        if 'Werkzeug' in server_header or 'Flask' in server_header:
            ...
            logger.info(f"⚠️ Servidor no identificado como Flask/Werkzeug: {server_header}")
        
        return True
    except requests.RequestException as e:
        logger.error(f"❌ No se puede acceder al servidor Flask en {BASE_URL}: {str(e)}")
        logger.info("⚠️ Asegúrate de que el servidor Flask esté ejecutándose con el comando:")
...
def main():
    ...
    try:
        ...
        if not login_success:
            logger.warning(f"❌ No se pudo iniciar sesión con las credenciales configuradas: {ADMIN_EMAIL}")
            ...
            if alt_success:
                login_success = True
                logger.info(f"✅ Inicio de sesión exitoso con credenciales alternativas: {alt_creds['email']}")
                logger.info("⚠️ Considera actualizar el archivo config_debug.py con estas credenciales")
            else:
                logger.error("❌ No se pudo iniciar sesión con ninguna credencial. Abortando.")
                logger.info("Sugerencias:")
                logger.info("1. Verifica que el servidor Flask esté ejecutándose correctamente")
                logger.info("2. Verifica que las credenciales en config_debug.py sean correctas")
                logger.info("3. Intenta obtener las credenciales correctas del administrador del sistema")
                return False
