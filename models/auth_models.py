...
class LoginAttempt:
    ...
    def record_attempt(self, email, success):
        try:
            now = datetime.now()
            attempt = {"email": email, "timestamp": now, "success": success}
                ...
                if recent_attempts >= self.max_attempts:
                    ...
                    self.users_collection.update_one(
                        {"email": email},
                        {
                            "$set": {
                                "locked_until": now
                                + timedelta(minutes=self.lockout_duration),
                                "updated_at": now,
                            }
                        },
                    )
        ...
        try:
            ...
            if "locked_until" in user:
                if user["locked_until"] > datetime.now():
                    ...
                    return (
                        True,
                        f"Cuenta bloqueada. Intente de nuevo en {int(remaining)} minutos",
                    )
                else:
                    # Desbloquear usuario
                    self.users_collection.update_one(
...
class SecurityLog:
    ...
    def log_security_event(
        self, event_type, user_id=None, email=None, details=None, ip_address=None
    ):
        try:
    ...
    def get_user_events(self, user_id, limit=50):
        try:
            events = list(
                self.security_log.find({"user_id": ObjectId(user_id)})
                .sort("timestamp", -1)
                .limit(limit)
            )
            return events
    ...
    def get_recent_events(self, event_type=None, limit=50):
        try:
            ...
            events = list(
                self.security_log.find(query).sort("timestamp", -1).limit(limit)
            )
            return events
        except Exception as e:
...
class UserProfile:
    ...
    def get_login_history(db, user_id, limit=10):
        return list(
            db.login_attempts.find({"user_id": ObjectId(user_id)})
            .sort("timestamp", -1)
            .limit(limit)
        )
