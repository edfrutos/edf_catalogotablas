        ...
        for directorio in directorios:
            ...
            try:
                ...
                if scripts_en_dir:
                    ...
                    print(f"    ⚠️  No se encontraron scripts")
                    
            except Exception as e:
                print(f"    ❌ Error accediendo al directorio: {e}")
        
        if not scripts_encontrados:
            print(f"  ⚠️  No se encontraron scripts en esta categoría")
        else:
        ...
        try:
            ...
            if result.returncode == 0:
                try:
                    ...
                    print(f"  ⚠️  Ejecutado pero no devolvió JSON válido")
            else:
                print(f"  ❌ Error en ejecución")
                print(f"     Error: {result.stderr}")
                
        except subprocess.TimeoutExpired:
            print(f"  ⚠️  Timeout (script de larga duración)")
        except Exception as e:
            print(f"  ❌ Error: {e}")
