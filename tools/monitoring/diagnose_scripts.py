...
def main():
    ...
    if os.path.exists(scripts_dir):
        subdirs = [d for d in os.listdir(scripts_dir) if os.path.isdir(os.path.join(scripts_dir, d))]
        for subdir in subdirs:
            subdir_path = os.path.join(scripts_dir, subdir)
            script_count = len(glob.glob(os.path.join(subdir_path, "*.sh"))) + len(glob.glob(os.path.join(subdir_path, "*.py")))
    ...
    if os.path.exists(maintenance_dir):
        scripts = glob.glob(os.path.join(maintenance_dir, "*.sh")) + glob.glob(os.path.join(maintenance_dir, "*.py"))
        for script in scripts:
            print(f"Script: {os.path.basename(script)}")
            print(f"  Ruta completa: {script}")
            print(f"  Existe: {'Sí' if os.path.exists(script) else 'No'}")
            print(f"  Permisos de ejecución: {'Sí' if os.access(script, os.X_OK) else 'No'}")
    ...
    if os.path.exists(tools_maintenance_dir):
        scripts = glob.glob(os.path.join(tools_maintenance_dir, "*.sh")) + glob.glob(os.path.join(tools_maintenance_dir, "*.py"))
        for script in scripts:
            print(f"Script: {os.path.basename(script)}")
            print(f"  Ruta completa: {script}")
            print(f"  Existe: {'Sí' if os.path.exists(script) else 'No'}")
    ...
    test_path = "/var/www/vhosts/edefrutos2025.xyz/httpdocs/scripts/maintenance/monitor_socket.sh"
    ...
    print("4. Ejecute el script copy_scripts.sh para copiar los scripts al directorio tools")
