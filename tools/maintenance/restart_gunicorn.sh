...
setup_signal_handlers() {
    ...
    trap 'd "Error en la línea $LINENO"' ERR
}
...
check_dependencies() {
    local missing=()
    for cmd in gunicorn ps lsof netstat; do
...
# Configurar manejo de señales
setup_signal_handlers() {
    trap 'log "INFO" "Señal recibida, terminando..."; cleanup; exit 0' INT TERM
}
...
# Función principal
main() {
    log "INFO" "=== Iniciando reinicio controlado de Gunicorn ==="
    ...
    log "INFO" "Deteniendo instancias previas de Gunicorn..."
    pkill -f "gunicorn.*${APP_MODULE}" || true
    sleep 2
    
    # Iniciar Gunicorn
    log "INFO" "Iniciando Gunicorn..."
