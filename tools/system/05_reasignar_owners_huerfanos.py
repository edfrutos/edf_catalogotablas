...
admin_owner = (
    input(
        "Introduce el email o username del usuario admin al que reasignar los huérfanos: "
    )
    .strip()
    .lower()
)
...
def user_exists(owner):
    return (
        db["users"].find_one({"$or": [{"email": owner}, {"username": owner}]})
        is not None
    )
...
user_usernames = set(
    u["username"].lower() for u in db["users"].find() if u.get("username")
)
        ...
        result = db[col].update_many(
            {"_id": {"$in": huérfanos}},
            {
                "$set": {
                    ...
                    "owner_name": admin_owner,
                }
            },
        )
        print(
            f"✔ Reasignados {result.modified_count} {label.lower()} huérfanos a '{admin_owner}'"
        )
    else:
