...
def test_mongodb_connection(uri):
    print(f"Probando conexión a MongoDB con URI: {uri[:30]}... (ocultando credenciales)")
    try:
...
def main():
    ...
    parser.add_argument("--mode", choices=["diagnostic", "monitor"], help="Modo de ejecución: diagnóstico rápido o monitor completo")
    parser.add_argument("--logs-dir", default="logs", help="Directorio de logs a analizar (solo modo monitor)")
    parser.add_argument("--email-alerts", action="store_true", help="Enviar alertas por email si hay problemas (solo modo monitor)")
    args = parser.parse_args()

    # Preguntar modo si no se pasa por argumento
    mode = args.mode
    if not mode:
        mode = input("¿Modo de ejecución? [diagnostic/monitor]: ").strip().lower() or "diagnostic"
