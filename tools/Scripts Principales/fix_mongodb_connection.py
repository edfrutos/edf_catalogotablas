...
def update_env_file(env_path, use_local_mongodb=True):
    ...
    if use_local_mongodb:
        ...
        backup_files = [f for f in os.listdir(os.path.dirname(env_path)) if f.startswith(os.path.basename(env_path) + '.bak')]
        if backup_files:
            ...
            mongo_uri_match = re.search(r'^MONGO_URI\s*=\s*(.*)$', backup_content, re.MULTILINE)
            if mongo_uri_match:
...
def update_app_py(app_path, disable_mongodb=True):
    ...
    if disable_mongodb:
        ...
        if "raise ValueError(\"MONGO_URI no está configurada en el archivo .env\")" in content:
...
def update_wsgi_file(wsgi_path, use_mock_data=True):
    ...
    if use_mock_data:
        ...
        # Restaurar el comportamiento original desde el backup
        backup_files = [f for f in os.listdir(os.path.dirname(wsgi_path)) if f.startswith(os.path.basename(wsgi_path) + '.bak')]
        if backup_files:
            # Usar el backup más reciente
            latest_backup = sorted(backup_files)[-1]
...
def main():
    ...
    print("Se ha implementado una solución temporal para los problemas de conexión a MongoDB.")
    print("La aplicación ahora debería funcionar con datos simulados en lugar de conectarse a MongoDB Atlas.")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--revert":
        ...
        print("Se han revertido los cambios y la aplicación intentará conectarse a MongoDB Atlas nuevamente.")
    else:
        main()
