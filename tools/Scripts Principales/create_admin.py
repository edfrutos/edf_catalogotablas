...
def create_admin_user():
    ...
    # Generar salt y hash
    salt = (
        base64.b64encode(b"adminsalt12345678").decode("utf-8").rstrip("=")
    )  # Salt fijo para reproducibilidad
    password_hash = (
        base64.b64encode(
            hashlib.scrypt(
    ...
    result = get_users_collection().replace_one(
        {"email": email}, admin_user, upsert=True
    )
    ...
    if result.upserted_id:
        ...
        print(
            f"Usuario admin actualizado. Documentos modificados: {result.modified_count}"
        )
