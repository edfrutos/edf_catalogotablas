...
def usuario_existe(email, username):
    return (
        users_col.find_one({"$or": [{"email": email}, {"username": username}]})
        is not None
    )
...
for u in usuarios:
    ...
    print(
        f"Migrado usuario de 'usuarios': {email} (contraseña temporal: {password_temp})"
    )
...
for u in usuarios_unificados:
    ...
    users_col.insert_one(user_doc)
    print(
        f"Migrado usuario de 'users_unified': {email} (contraseña temporal: {password_temp})"
    )
