...
def migrate_user(user_id):
    try:
        ...
        merged_user = {
            "_id": user_id,
            "nombre": (user or {}).get(
                "nombre", (unified or {}).get("nombre", (unified or {}).get("name", ""))
            ),
        ...
        if password:
            ...
            if password.startswith("scrypt:"):
                ...
                logger.warning(
                    f"Formato de contraseña desconocido para {merged_user['email']}"
                )
                merged_user["password"] = password
                merged_user["password_type"] = "unknown"

        # Actualizar en la colección users
        result = users.update_one({"_id": user_id}, {"$set": merged_user}, upsert=True)
        ...
        if result.modified_count > 0 or result.upserted_id:
            ...
            if unified:
                ...
                logger.info(
                    f"Usuario eliminado de users_unified: {merged_user['email']}"
                )
