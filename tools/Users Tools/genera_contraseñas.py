...
def generar_y_guardar_contraseñas():
    ...
    contraseñas = []
    while True:
        nombre_contraseña = input(
            "Ingrese el nombre para la contraseña (o 'salir' para terminar): "
        )
    ...
    if contraseñas:
        guardar_o_no = input(
            "¿Desea guardar las contraseñas en un archivo CSV? (s/n): "
        )
        if guardar_o_no.lower() == "s":
            nombre_archivo = input(
                "Ingrese el nombre del archivo (o presione Enter para usar 'contraseñas.csv'): "
            )
            # Usar nombre por defecto si está vacío
            nombre_archivo = nombre_archivo or "contraseñas.csv"
