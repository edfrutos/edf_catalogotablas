...
for email, us in email_map.items():
    if len(us) > 1:
        ...
        print(
            f"✔ Eliminados {len(us) - 1} usuarios duplicados con email '{email}' (se mantiene _id: {to_keep})"
        )
...
for username, us in username_map.items():
    if len(us) > 1:
        ...
        for u in us[1:]:
            ...
            deleted_ids.add(str(u["_id"]))
        print(
            f"✔ Eliminados {len(us) - 1} usuarios duplicados con username '{username}' (se mantiene _id: {to_keep})"
        )
