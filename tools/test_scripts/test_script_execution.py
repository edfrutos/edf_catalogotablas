    ...
    for script in test_scripts:
        ...
        try:
            ...
            if not os.path.exists(runner_path):
                print(f"    ❌ script_runner.py no encontrado")
                continue
...
def main():
    ...
    print(f"1. {'✅' if service_restarted else '❌'} El servicio edefrutos2025 se reinició {'correctamente' if service_restarted else 'con errores'}")
    print(f"2. {'✅' if scripts_executable else '❌'} Los scripts son {'accesibles y ejecutables' if scripts_executable else 'no accesibles o ejecutables'}")
