...
parser = argparse.ArgumentParser(description="Verificación simple de conexión a MongoDB. No requiere argumentos.")
...
def check_mongodb_status():
    ...
    try:
        ...
        if 'mongod' in ps_result.stdout:
            ...
            print("  ℹ️ No se encontraron procesos de MongoDB en ejecución (normal si se usa MongoDB Atlas)")
        ...
        if os.path.exists(log_path):
            tail_result = subprocess.run(['tail', '-n', '50', log_path], capture_output=True, text=True)
            ...
            if 'MongoDB connection error' in tail_result.stdout:
                ...
                print("  ✅ La aplicación se conectó correctamente a MongoDB según los logs recientes")
                return True
...
def main():
    ...
    if network_ok and app_mongo_ok:
        ...
        print("⚠️ Hay conectividad de red a MongoDB, pero la aplicación puede tener problemas de conexión")
        return True
    else:
