...
def verificar_s3_completo():
    ...
    for i, imagen in enumerate(imagenes_test, 1):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"      ❌ [{i}/{len(imagenes_test)}] {imagen} - Status: {response.status_code}"
                )
    ...
    print(
        f"   🔑 AWS Access Key: {'✅ Configurada' if aws_access_key else '❌ No configurada'}"
    )
    print(
        f"   🔑 AWS Secret Key: {'✅ Configurada' if aws_secret_key else '❌ No configurada'}"
    )
